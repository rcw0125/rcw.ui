...
namespace RV.UI
{
    public partial class FrmBtnMag : Form
    {
        ...
        private void FrmQR_ANQX_Load(object sender, EventArgs e)
        {
            ...
            btnAllResource = TS_MODULE.GetList("C_MODULE_TYPE=3");
            loadMenu();
        }
        public void loadMenu()
        {
            //C_MODULE_TYPE IS '模块类型：1-普通模块;2-系统模块;3-按钮模块'
            //查询非按钮菜单             
            userResource = menuMag.GetPriviledgeByUserAccount(UserInfo.UserAccount);

            menuResource=userResource.Where(o => (o.C_MODULE_TYPE == "1" || o.C_MODULE_TYPE == "2")).OrderBy(o => o.N_ORDER).ToList();
        ...
        {
            btnResource = TS_MODULE.GetList("C_MODULE_TYPE='3' and C_PARENT_ID=@C_PARENT_ID order by N_ORDER", tag);
