...
namespace Rcw.Model
{
	...
	public class TS_MODULE : DbEntity
	{
		...
		private string _c_module_type;	
		/// <summary>
		/// 模块类型：1-普通模块;2-系统模块;3-按钮模块
        /// </summary>		
				
		[DisplayName("模块类型：1-普通模块;2-系统模块;3-按钮模块")]
        public string C_MODULE_TYPE
        {
            get
            {
            	return _c_module_type; 
            }
            set
            {
                if (_c_module_type != value)
                {
                   _c_module_type = value;
                   RaisePropertyChanged("C_MODULE_TYPE", true);	                   
                }
            }
        }        
        ...
        public static int GetMaxID()
        {
            ...
            strSql.Append("select max(C_ID) FROM TS_MODULE where C_MODULE_TYPE <> '3' ");
